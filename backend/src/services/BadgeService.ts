...
export class BadgeService {
  ...
  private subscriberBadgesCache = new Map<string, Map<number, SubscriberBadge>>();

  async loadChannelBadges(channelName: string): Promise<ChannelInfo> {
    try {
      const response = await axios.get(`${this.KICK_API_BASE}/channels/${channelName}`, {
        headers: {
      ...
      this.cacheSubscriberBadges(channelName, channelInfo.subscriber_badges);

      console.log(`Loaded ${channelInfo.subscriber_badges.length} subscriber badges for ${channelName}`);
      return channelInfo;
    } catch (error) {
      if (axios.isAxiosError(error)) {
        console.error(`Failed to load badges for channel ${channelName}:`, {
        ...
        if (error.response?.status === 403) {
          console.error('Request blocked by Kick - possible rate limiting or bot detection');
        } else if (error.response?.status === 404) {
          console.error(`Channel '${channelName}' not found`);
        }
      } else {
        console.error(`Unexpected error loading badges for channel ${channelName}:`, error);
      }
      throw error;
    }
  }

  private cacheSubscriberBadges(channelName: string, badges: ChannelInfo['subscriber_badges']): void {
    const badgeMap = new Map<number, SubscriberBadge>();
    
    badges.forEach(badge => {
      const imageUrl = badge.badge_image.src;
      if (imageUrl && badge.months) {
        badgeMap.set(badge.months, {
          id: badge.id,
          months: badge.months,
          url: imageUrl
        });
    ...
    this.subscriberBadgesCache.set(channelName, badgeMap);
    console.log(`Cached ${badgeMap.size} subscriber badges for ${channelName}`);
  }

  processUserBadges(kickBadges: KickBadge[], channelName: string): Badge[] {
    const badges: Badge[] = [];

    kickBadges.forEach(kickBadge => {
      let badge: Badge;

      switch (kickBadge.type) {
        case 'subscriber':
          badge = this.processSubscriberBadge(kickBadge, channelName);
          break;
        case 'sub_gifter':
        case 'gifted_subs':
          badge = this.processGiftedSubBadge(kickBadge);
          break;
        default:
          badge = this.processStandardBadge(kickBadge);
          break;
      }
  ...
  private processSubscriberBadge(kickBadge: KickBadge, channelName: string): Badge {
    const channelBadges = this.subscriberBadgesCache.get(channelName);
    const count = kickBadge.count;
    
    console.log(`Processing subscriber badge for ${channelName}:`, {
      count,
      hasChannelBadges: !!channelBadges,
      channelBadgesSize: channelBadges?.size || 0,
      availableMonths: channelBadges ? Array.from(channelBadges.keys()) : []
    });
    
    if (channelBadges && count !== undefined) {
      // Find exact match or highest qualifying badge
      const exactMatch = channelBadges.get(count);
      if (exactMatch) {
        console.log(`Found exact match for ${count} months: ${exactMatch.url}`);
        return {
          type: 'subscriber',
          image: exactMatch.url,
      ...
      // Find highest qualifying badge
      const availableMonths = Array.from(channelBadges.keys()).sort((a, b) => b - a);
      const qualifyingMonth = availableMonths.find(months => count >= months);
      
      if (qualifyingMonth) {
        const qualifyingBadge = channelBadges.get(qualifyingMonth)!;
        console.log(`Found qualifying badge for ${count} months (${qualifyingMonth}): ${qualifyingBadge.url}`);
        return {
          type: 'subscriber',
          image: qualifyingBadge.url,
          alt: `${count} month subscriber`,
          isCustom: true,
          count: count
        };
    ...
    console.log(`No custom badge found for ${count} months, using default`);
    // Fallback to default subscriber badge
    const badge: Badge = {
      type: 'subscriber',
      image: this.getDefaultBadgeSVG('subscriber'),
      alt: kickBadge.text || 'Subscriber',
      isCustom: false
    };
  ...
  private processGiftedSubBadge(kickBadge: KickBadge): Badge {
    const badge: Badge = {
      type: 'sub_gifter',
      image: this.getDefaultBadgeSVG('sub_gifter'),
      alt: kickBadge.text || `Gifted ${kickBadge.count || 0} subs`,
      isCustom: false
    };

    // Only add count if it exists
    if (kickBadge.count !== undefined) {
      badge.count = kickBadge.count;
    }

    return badge;
  }

  private processStandardBadge(kickBadge: KickBadge): Badge {
    const badge: Badge = {
      type: kickBadge.type,
      image: this.getDefaultBadgeSVG(kickBadge.type),
      alt: kickBadge.text || kickBadge.type,
      isCustom: false
    };
  ...
  private getDefaultBadgeSVG(badgeType: string): string {
    ...
    return svgBadges[badgeType] || '🎖️';
  }

  // Cache subscriber badges from frontend data
  cacheSubscriberBadgesFromFrontend(channelName: string, badges: any[]): void {
    console.log(`Caching subscriber badges for ${channelName}:`, badges);
    const badgeMap = new Map<number, SubscriberBadge>();
    
    badges.forEach((badge, index) => {
      console.log(`Processing badge ${index}:`, badge);
      let imageUrl = null;
      
      // Try different ways to get the image URL
      if (badge.badge_image) {
        if (badge.badge_image.srcset) {
          const srcset = badge.badge_image.srcset;
          const urls = srcset.split(',').map((s: string) => s.trim());
          const highestQuality = urls[urls.length - 1];
          imageUrl = highestQuality.split(' ')[0];
        } else if (badge.badge_image.url) {
          imageUrl = badge.badge_image.url;
        } else if (badge.badge_image.src) {
          imageUrl = badge.badge_image.src;
        } else if (typeof badge.badge_image === 'string') {
          imageUrl = badge.badge_image;
        }
      }
      
      console.log(`Badge ${index} image URL: ${imageUrl}, months: ${badge.months}`);
      ...
      if (imageUrl && badge.months) {
        ...
        console.log(`Cached ${badge.months}-month subscriber badge: ${imageUrl}`);
      } else {
        console.log(`Skipping badge ${index} - missing image URL or months`);
      }
    });

    this.subscriberBadgesCache.set(channelName, badgeMap);
    console.log(`Total subscriber badges cached for ${channelName}: ${badgeMap.size}`);
    console.log(`Cached months:`, Array.from(badgeMap.keys()));
  }
  getChannelSubscriberBadges(channelName: string): SubscriberBadge[] {
    const badgeMap = this.subscriberBadgesCache.get(channelName);
    return badgeMap ? Array.from(badgeMap.values()) : [];
  }

  // Clear cache (useful for testing or memory management)
  clearCache(): void {
    this.subscriberBadgesCache.clear();
