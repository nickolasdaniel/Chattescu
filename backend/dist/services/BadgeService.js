...
class BadgeService {
    constructor() {
        ...
        this.subscriberBadgesCache = new Map();
    }
    async loadChannelBadges(channelName) {
        try {
            const response = await axios_1.default.get(`${this.KICK_API_BASE}/channels/${channelName}`, {
                headers: {
    ...
    cacheSubscriberBadges(channelName, badges) {
    ...
    processUserBadges(kickBadges, channelName) {
        const badges = [];
    ...
    processSubscriberBadge(kickBadge, channelName) {
    ...
    processGiftedSubBadge(kickBadge) {
        const badge = {
            type: 'sub_gifter',
    ...
    processStandardBadge(kickBadge) {
        const badge = {
            type: kickBadge.type,
    ...
    cacheSubscriberBadgesFromFrontend(channelName, badges) {
        const badgeMap = new Map();
        badges.forEach(badge => {
            let imageUrl = null;
    ...
    getChannelSubscriberBadges(channelName) {
